...
function generate_game_state_json()
{
  ...
  state.reminders = [];
  reminders = document.getElementById("reminderLayer").getElementsByClassName("reminder");
...
{
  let reminders = document.getElementById("reminderLayer").getElementsByClassName("reminder");
  for (i = reminders.length - 1; i != -1; --i)
  {
    if (reminders[i].getAttribute("uid").substring(0, UID_LENGTH) == uid)
    {
      document.getElementById("reminderLayer").removeChild(reminders[i]);
    }
  }
}
...
function unprompt_reminders()
{
  ...
  tokens = document.getElementById("reminderLayer").children;
  for (var i = 0; i < tokens.length; i++)
  {
    var element = tokens[i];
    document.getElementById(element.id + "_img").style.display = "none";
...
function spawnReminder(roleName, reminder, uid, left, top)
{
  ...
  document.getElementById("reminderLayer").appendChild(div);
  makeDraggable(div);
  if (!loading) { save_game_state(); }
}
